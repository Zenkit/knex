...
class Client_PG extends Client {
  ...
  positionBindings(sql) {
    ...
    const jsEscapeCharacter = '\\';
    const postgresValueBoundaryCharacter = "'";
    for (let i = 0; i < sql.length; i++) {
      ...
      if (char === '?') {
        ...
        const escaped =
          sql[i - 1] === jsEscapeCharacter && sql[i - 2] !== jsEscapeCharacter;
        if (escaped) {
          // remove escape character
          adjustedSQL = adjustedSQL.substring(0, adjustedSQL.length - 1);
          adjustedSQL += '?';
        } else {
          questionCount++;
          adjustedSQL += `$${questionCount}`;
        }
        continue;
      }
      if (char === postgresValueBoundaryCharacter) {
